  ...
  const benefits = [
    {
      ...
      description: "Work independently and keep what you earn — no middlemen taking commissions.",
      color: "from-emerald-500 to-green-400"
    },
    {
      icon: Eye,
      title: "More Visibility",
      description: "Get discovered by high-intent users looking for your service in real-time.",
      color: "from-blue-500 to-cyan-400"
    },
    {
      icon: Clock,
      title: "Flexible Scheduling",
      description: "Work on your own terms — choose your working hours and availability.",
      color: "from-purple-500 to-pink-400"
    },
    {
      icon: MessageSquare,
      title: "Direct Communication",
      description: "Chat with clients, accept or reject requests, and build long-term relationships.",
      color: "from-orange-500 to-yellow-400"
    },
    {
      icon: TrendingUp,
      title: "Growth Support",
      description: "Get access to reviews, ratings, and tools to help grow your reputation.",
          ...
          {benefits.map((benefit, index) => {
            ...
            return (
              <div
                ...
                style={{
                  transitionDelay: isVisible ? `${(index - currentBenefit + 1) * 50}ms` : '0ms'
                }}
              >
                {/* Animated Background */}
                <div className={`absolute inset-0 rounded-2xl bg-gradient-to-r ${benefit.color} opacity-0 transition-opacity duration-700 ${
                  isActive ? 'opacity-10' : ''
                }`} />
                
                {/* Connection Line */}
                {index < benefits.length - 1 && (
                  <div className={`absolute left-8 top-20 w-0.5 h-16 bg-gradient-to-b transition-all duration-700 ${
                    isVisible ? 'from-blue-500/50 to-transparent' : 'from-transparent to-transparent'
                  }`} />
                )}
                
                {/* Main Card */}
                <div className={`relative bg-gray-900/50 backdrop-blur-sm p-8 rounded-2xl border transition-all duration-600 ${
                  isActive 
                  ...
                  <div className="flex items-start gap-6">
                    ...
                    <div className={`relative shrink-0 transition-all duration-500 ${
                      ...
                      <div className={`w-16 h-16 rounded-xl bg-gradient-to-br ${benefit.color} p-0.5 transition-all duration-500`}>
                      ...
                      {/* Pulse Effect */}
                      {isActive && (
                        <div className={`absolute inset-0 rounded-xl bg-gradient-to-br ${benefit.color} opacity-20 animate-pulse`} />
                      )}
                    </div>
          ...
          {benefits.map((benefit, index) => (
            <div
              ...
              className={`w-1 rounded-full transition-all duration-500 relative ${
                index === currentBenefit ? 'bg-blue-500 h-16 w-2' : 
                index < currentBenefit ? 'bg-blue-400/70 h-8' : 'bg-gray-700 h-4'
              }`}
            >
              {index === currentBenefit && (
                <div className={`absolute inset-0 rounded-full bg-gradient-to-b ${benefit.color} opacity-50 animate-pulse`} />
              )}
            </div>
          ))}
        </div>
